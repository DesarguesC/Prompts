...
def get_cond_pointE(opt, cond_image, cond_inp_type='pointE', cond_model=None):
    if isinstance(cond_image, str):
    ...
    if cond_inp_type == 'pointE':
        ...
        samples = None
        for x in tqdm(sampler.sample_batch_progressive(batch_size=1, model_kwargs=dict(images=[pointE]))):
            samples = x
